...
async function main() {
  ...
  while (true) {
    let unreadMessages = await gsm.readSMS(
    ...
    if (unreadMessages.length > 0) {
      ...
      await gsm.deleteAllMessages(
        GSM.MessageStorage.sim,
        GSM.MessageDeleteFilter.readSentAndUnsent
      );
    }
    await timeout(10000);
  }
...
async function getPhoneNumber(gsm) {
  try {
    ...
    try {
      const result = await gsm.readPhoneBook(
        GSM.PhoneBookStorage.ownNumber,
        1,
        1
      );
